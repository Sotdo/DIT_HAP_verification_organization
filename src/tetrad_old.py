...
class RobustOffsetGridRestorer:
    ...
    def fit_transform(self, df):
        """
        Fit and transform a DataFrame with colony coordinates to a regular grid.
        
        Parameters:
        -----------
        df : pd.DataFrame
            DataFrame with 'centroid_x' and 'centroid_y' columns
            
        Returns:
        --------
        result_df : pd.DataFrame
            Input DataFrame with added 'row' and 'col' columns indicating grid position
        restored_grid : np.ndarray
            Sorted array of shape (rows, cols, 2) with grid coordinates
        """
        ...
        restored_grid_flat = grid_aligned.dot(R_inv.T)
        
        # Reshape and sort the grid: shape (cols, rows, 2)
        # First dimension is columns, second dimension is rows
        restored_grid = restored_grid_flat.reshape(self.rows, self.cols, 2).transpose(1, 0, 2)
        
        # Calculate distance threshold for outlier detection
        # Use a fraction of the average grid spacing
        distance_threshold = max(f_sx, f_sy) * 0.5
        
        # Assign each input point to nearest grid point
        result_df['row'] = np.nan
        result_df['col'] = np.nan
        
        for i, point in enumerate(points):
            # Calculate distances to all grid points
            distances = np.sqrt(np.sum((restored_grid_flat - point)**2, axis=1))
            min_distance = np.min(distances)
            
            # Only assign if within threshold (not an outlier)
            if min_distance <= distance_threshold:
                min_idx = np.argmin(distances)
                
                # Convert flat index to row, col
                row_idx = min_idx // self.cols
                col_idx = min_idx % self.cols
                
                result_df.iloc[i, result_df.columns.get_loc('row')] = row_idx
                result_df.iloc[i, result_df.columns.get_loc('col')] = col_idx
        
        return result_df, restored_grid
